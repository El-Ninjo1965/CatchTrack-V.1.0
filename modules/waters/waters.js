...
typeof CatchTrackDatabase.isReady ===
"function" &&
...
try {
    const result =
        await this.reverseGeocodeBigDataCloud(
            lat,
            lon
        );
    if (result) {
    ...
    this.handleError(
        error,
        "waters:reverse-geocode:bigdatacloud"
    );
}
try {
    const result =
        await this.reverseGeocodeNominatim(
            lat,
            lon
        );
    if (result) {
    ...
    this.handleError(
        error,
        "waters:reverse-geocode:nominatim"
    );
}
return null;
